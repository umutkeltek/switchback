...
Switchback is a local-first **AI execution gateway** in Rust: one binary that normalizes every AI call into a **canonical typed IR** (`sb-core`), routes it with an **explainable `RouteDecision`** + fallback, and streams it back in the client's format. Crate graph: `sb-core` ← {`sb-adapter`, `sb-protocols`, `sb-router`, `sb-credentials`, `sb-compress`, `sb-ledger`, `sb-trace`} ← `sb-adapters` ← `sb-runtime` ← `sb-server` (binary `switchback`); `sb-store` (SQLite revisions/audit/usage, no sb deps) is a dep of both `sb-runtime` and `sb-ledger`; `sb-plugin` (Plugin trait + trusted built-ins) is a `sb-runtime` dep. `sb-runtime` owns the execution `Engine` + the immutable revisioned `CompiledSnapshot` (per-request snapshot pinning, atomic hot-reload, durable revision/audit history via `sb-store`, the `PluginHost` compiled from `config.plugins`); `sb-ledger` dual-writes usage to `sb-store` and hydrates its summary at startup so `/v1/usage` survives restarts; `sb-server` is HTTP ingress/egress + protocol translation over `Engine::execute`. Design docs are in `docs/` (git-ignored, private).
