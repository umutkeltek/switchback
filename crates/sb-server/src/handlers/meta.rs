...
pub(crate) async fn usage(State(state): State<AppState>) -> Json<serde_json::Value> {
...
pub(crate) async fn trace_by_id(State(state): State<AppState>, Path(id): Path<String>) -> Response {
