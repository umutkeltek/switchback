...
fn command_schema_json() -> serde_json::Value {
    serde_json::json!({
        ...
        "commands": [
            ...
            {"name": "setup native-relay audit", "writes_config": false, "output": "text or JSON with --json; inspects local Codex/Claude Code installation and auth-store shape without printing token values", "example": "switchback --json setup native-relay audit"},
            {"name": "setup native-relay capture", "writes_config": true, "output": "text or JSON with --json; sanitizes a raw native-client debug/HTTP capture into a relay fixture file", "example": "switchback --json setup native-relay capture --client codex --fixture non_stream_request_response --from-file raw.log --out-file crates/sb-protocols/tests/fixtures/native-relay/codex/non_stream_request_response.json"},
            ...
            {"name": "native status", "writes_config": false, "output": "text or JSON with --json; read-only native Codex/Claude Code setup, token-source shape, lane separation, server health, and possible local runtime proxy helpers; helper ids are redacted unless --show-local-ids is passed", "example": "switchback --json native status --config switchback.yaml"},
            {"name": "native accounts reconcile", "writes_config": true, "output": "metadata-only provider-account reconciliation; --dry-run performs zero writes", "example": "switchback --json native accounts reconcile --dry-run"},
            {"name": "native accounts list/resolve/merge/split", "writes_config": false, "output": "canonical provider-account authority and explicit adjudication", "example": "switchback --json native accounts list"},
            {"name": "native verify", "writes_config": false, "output": "text or JSON with --json; strict native preflight plus optional synthetic tap exercises for large-payload, stream, and websocket behavior; never sends prompts or credentials to external providers", "example": "switchback --json native verify --exercise large-payload --exercise stream --exercise websocket --config switchback.yaml"},
            {"name": "native profiles list", "writes_config": false, "output": "text or JSON with --json; lists configured Codex/Claude Code profile ids, modes, model aliases, pinned account refs, and readiness", "example": "switchback --json native profiles list --config switchback.yaml"},
            {"name": "native profiles doctor", "writes_config": false, "output": "text or JSON with --json; diagnoses one named native profile without printing token values", "example": "switchback --json native profiles doctor codex-work --config switchback.yaml"},
            {"name": "native profiles env", "writes_config": false, "output": "text or JSON with --json; prints safe env/header hints for one named profile", "example": "switchback --json native profiles env codex-work --config switchback.yaml"},
            {"name": "native import-history", "writes_config": false, "output": "text or JSON with --json; metadata-only dry-run or apply into server.state_store/--state-store; no prompt/response content or exact local paths are persisted, and displayed local paths are redacted unless --show-local-paths is passed", "example": "switchback --json native import-history --dry-run"},
            ...
            {"name": "lane audit codex-scout", "writes_config": false, "output": "text or JSON with --json; checks Codex's local scout profile/provider tables against the Switchback scout/code lane contract", "example": "switchback --json lane audit codex-scout --config switchback.yaml --codex-config ~/.codex/config.toml"},
            {"name": "lane install codex-scout", "writes_config": true, "output": "text or JSON with --json; writes or repairs Codex's local scout profile/provider tables and backs up the existing config", "example": "switchback --json lane install codex-scout --config switchback.yaml --codex-config ~/.codex/config.toml"},
