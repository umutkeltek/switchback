...
fn command_schema_json() -> serde_json::Value {
    serde_json::json!({
        ...
        "commands": [
            ...
            {"name": "native import-history", "writes_config": false, "output": "text or JSON with --json; metadata-only dry-run over native Codex/Claude Code history stores; no prompt/response content is emitted and local paths are redacted unless --show-local-paths is passed", "example": "switchback --json native import-history --dry-run"},
